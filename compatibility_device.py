...
# a bit unsure about US layouts and HID usage 31
# now producing 0x32 usage for key_backslash
# not sure if 0x31 need to be produced for us backslash.
# also ecodes.KEY_102ND
...
class CompatibilityModeDevice:
    def __init__(self, loop: asyncio.AbstractEventLoop, device_path):
        ...
        self.hidraw_device.rdesc = bytearray.fromhex(
            "05010906a1018501050719e029e715002501750195088102950175088103950575"
            "010508190129059102950175039103950675081500256d05071900296d8100c005"
            "0C0901A1018502050C150025017501950709B509B609B709CD09E209E909EA8102"
            "95018101C0"
        )
        ...
        async for ev in self.ev_device.async_read_loop():
            if ev.type == ecodes.EV_KEY and ev.value < 2:
                ...
                if ev.code in CONSUMER_KEYS_EVENT_TO_USAGE_FLAG_MAPPING:
                    ...
                    if (ev.code not in
                        NORMAL_KEYS_EVENT_TO_USAGE_FLAG_MAPPING) and (
                            ev.code not in
                            MODIFIER_KEYS_EVENT_TO_USAGE_FLAG_MAPPING
                    ):
                        continue
    ...
    def __send_consumer_hid_report(self):
        # first byte - report id 2,
        # second byte - consumer key flags and one bit is constant (0x80)
        report = bytearray(b'\x02\x80')
        for code in self.pressed_consumer_keys:
            report[1] |= CONSUMER_KEYS_EVENT_TO_USAGE_FLAG_MAPPING[code]
        self.hidraw_device.call_input_event(report)

    def __send_normal_hid_report(self):
        # first byte - report id 1,
        # then 1 byte with modifier key flags
        # then const byte with 0
        # then 6 bytes with up to 6 keycodes (0 to 109)
        ...
        for code in self.pressed_keys:
            if code in MODIFIER_KEYS_EVENT_TO_USAGE_FLAG_MAPPING:
                report[1] |= MODIFIER_KEYS_EVENT_TO_USAGE_FLAG_MAPPING[code]
            else:
                report[key_index] = NORMAL_KEYS_EVENT_TO_USAGE_FLAG_MAPPING[
                    code]
                key_index += 1
                if key_index > 8:
                    break
