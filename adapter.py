...
class BluetoothAdapter:
    ...
    def wait_till_adapter_present_then_init_sync(self):
        ...
        asyncio.run_coroutine_threadsafe(
            self.wait_till_adapter_present_then_init(),
            loop=self.loop
        )
    ...
    async def wait_till_adapter_present_then_init(self):
        ...
        self.register_agent()
        self.adapter = self.bus.get_proxy(
            service_name="org.bluez",
            object_path=ADAPTER_OBJECT,
        ...
        while not self.powered:
            ...
            try:
                ...
                if self.powered:
                    ...
                    print(
                        "Failed to turn on. "
                        "Please turn on Bluetooth in the system"
                    )
            except Exception:
                print(
    ...
    def interfaces_removed(self, obj_name, interfaces):
        if obj_name == ADAPTER_OBJECT or obj_name == ROOT_OBJECT:
            ...
            asyncio.run_coroutine_threadsafe(self.init(), loop=self.loop)
        elif INPUT_HOST_INTERFACE in interfaces or (
                INPUT_DEVICE_INTERFACE in interfaces
        ):
    ...
    def register_agent(self):
        if not self.agent_published:
            ...
            agent_manager = self.bus.get_proxy(
                service_name="org.bluez",
    ...
    def start_scan(self):
        if self.adapter is not None:
            ...
            #   {"UUIDs":dt.get_variant(dt.List[dt.Str], [UUID])})
    ...
    async def __shutdown_scanning(self):
        while all((
                ...
                self.scan_start_time is not None,
                (self.scan_start_time + timedelta(seconds=60)) > datetime.now()
        )):
    ...
    async def __shutdown_discoverable(self):
        while all((
            self.adapter is not None,
            self.discoverable,
            self.discoverable_start_time is not None,
            self.discoverable_start_time + timedelta(
                seconds=120) > datetime.now()
        )):
            await asyncio.sleep(1)
