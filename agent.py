...
class Agent(object):
    ...
    def DisplayPasskey(self, device: dt.ObjPath, passkey: dt.UInt32,
                       entered: dt.UInt16):
        print("DisplayPasskey (%s, %06u entered %u)" %
              (device, passkey, entered))
        self.on_agent_action(
            {'action': 'display_passkey', 'passkey': passkey, 'device': device,
             'entered': entered})

    def DisplayPinCode(self, device: dt.ObjPath, pincode: dt.Str):
        print("DisplayPinCode (%s, %s)" % (device, pincode))
        self.on_agent_action({'action': 'display_pin_code', 'pincode': pincode,
                              'device': device})
    ...
    def RequestConfirmation(self, device: dt.ObjPath, passkey: dt.UInt32):
        ...
        self.on_agent_action(
            {'action': 'confirm_passkey', 'passkey': str(passkey).zfill(6),
             'device': device})

    def request_confirmation_response(self, device, passkey, confirmed):
        if self.request_confirmation_device == device and (
            passkey == self.request_confirmation_passkey and confirmed
        ):
            set_trusted(device)
        else:
            self.request_confirmation_device = None
