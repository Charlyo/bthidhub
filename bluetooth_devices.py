...
class BluetoothDevice:
    ...
    async def connect_sockets(self):
        if self.sockets_connected or (
                self.control_socket_path is None
        ) or self.interrupt_socket_path is None:
            return

        print("Connecting sockets for ", self.object_path)
        if not self.connected:
            print(
                "BT Device is not connected. No point connecting sockets. "
                "Skipping."
            )
        try:
            ...
            asyncio.run_coroutine_threadsafe(self.loop_of_fun(True),
                                             loop=self.loop)
            asyncio.run_coroutine_threadsafe(self.loop_of_fun(False),
                                             loop=self.loop)
        except Exception as err:
            print("Error while connecting sockets for ", self.object_path,
                  ". Will retry in a sec", err)
            try:
            ...
            asyncio.run_coroutine_threadsafe(self.connect_sockets(),
                                             loop=self.loop)
    ...
    async def loop_of_fun(self, is_ctrl):
        ...
        while sock is not None:
            try:
                ...
                print("Cannot read data from socket. ", self.object_path,
                      "Closing sockets")
                if self is not None:
                    try:
                        self.disconnect_sockets()
    ...
    def device_connected_state_changed(self, arg1, arg2, arg3):
        ...
        if self.device_registry.on_devices_changed_handler is not None:
            asyncio.run_coroutine_threadsafe(
                self.device_registry.on_devices_changed_handler(),
                loop=self.loop
            )
...
class BluetoothDeviceRegistry:
    ...
    def add_device(self, device_object_path, is_host):
        ...
        p = self.bus.get_proxy(
            ...
            interface_name=(
                INPUT_HOST_INTERFACE if is_host else INPUT_DEVICE_INTERFACE)
        )
    ...
    def remove_device(self, device_object_path):
        ...
        list = (
            self.connected_hosts
            if device.is_host else self.connected_devices
        )
        if device in list:
            list.remove(device)
        device.finalise()
    ...
    def switch_host(self):
        self.current_host_index = (self.current_host_index + 1) % len(
            self.connected_hosts)
    ...
    def send_message(self, msg, send_to_hosts, is_control_channel):
        if (IGNORE_INPUT_DEVICES and not send_to_hosts) and (
                not is_control_channel and self.hid_devices is not None
        ):
            asyncio.run_coroutine_threadsafe(
                self.hid_devices.send_message_to_devices(msg), loop=self.loop)
            return
        targets: List[BluetoothDevice] = (
            self.__get_current_host_as_list()
            if send_to_hosts else self.connected_devices
        )
        for target in list(targets):
            try:
                socket = (
                    target.control_socket if is_control_channel else
                    target.interrupt_socket
                )
                socket.sendall(msg)
            except Exception:
