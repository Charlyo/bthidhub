...
class HIDDevice:
    ...
    def set_device_filter(self, device_filter):
    ...
    def hidraw_event(self):
        ...
        # Scroll lock key in QuickFire TK
    ...
    def indicate_switch_with_mouse_movement(self):
        """Move mouse in circular direction so user can see which host is
        active now"""
        for i in range(3):
            # Up
...
class HIDDeviceRegistry:
    ...
    def __scan_devices(self):
        ...
        filtered_devices = filter(
            lambda d: 1 in d.capabilities().keys() and 1 in d.capabilities()[
                1] and d.info.bustype != 0x06,
            evdevs
        )
        for dev in filtered_devices:
            compatibility_mode = (
                (DEVICES_CONFIG_COMPATIBILITY_DEVICE_KEY in
                    self.devices_config) and (dev.path in self.devices_config[
            ...
            if compatibility_mode and (
                    dev.path not in self.compatibility_mode_devices):
                self.compatibility_mode_devices[
                    dev.path] = CompatibilityModeDevice(self.loop, dev.path)
        ...
        for dev_path in self.compatibility_mode_devices:
            if len([
                ...
                if d["path"] == dev_path and d["compatibility_mode"]
            ]) == 0:
        ...
        HID_DEVICES_PATH = '/sys/bus/hid/devices'
        for device in os.listdir(HID_DEVICES_PATH):
            device_path = os.path.join(HID_DEVICES_PATH, device)
            device_input_path = os.path.join(device_path, 'input')
            device_hidraw_path = os.path.join(device_path, 'hidraw')
            device_uevent_path = os.path.join(device_path, 'uevent')
            try:
                with open(device_uevent_path, 'r') as uevent:
                    name = re.search('HID_NAME\s*=(.+)', uevent.read()).group(1)
                    hidraw = os.listdir(device_hidraw_path)[0]
                    inputs = os.listdir(device_input_path)
                    events = []
                    ...
                    for input in inputs:
                        input_events = [
                            e
                            for e in os.listdir(
                                os.path.join(device_input_path, input)
                            )
                        ...
                        for event in input_events:
                            for input_device in self.input_devices:
                                if all((
                                    input_device["compatibility_mode"],
                                    input_device["path"].find(event) >= 0
                                )):
                ...
                print("Error while loading HID device: ", device, ", Error: ",
                      exc, ", Skipping.")
        devs_to_remove = []
        for dev in self.capturing_devices:
            if dev not in devs_dict or not (
                    self.__is_configured_capturing_device(devs_dict[dev])
            ) or dev in devs_in_compatibility_mode:
        ...
        for dev in devs:
            if dev["instance"] not in self.capturing_devices and (
                    self.__is_configured_capturing_device(dev["id"])
            ) and dev not in devs_in_compatibility_mode:
                # create capturing device
                self.capturing_devices[dev["instance"]] = HIDDevice(
                    dev,
                    self.__get_configured_device_filter(dev["id"]),
                    self.loop,
                    self
                )
        self.devices = devs
    ...
    def set_device_filter(self, device_id, filter_id):
        ...
        self.__save_config()
        device_filter = self.__get_configured_device_filter(device_id)
        for dev in self.capturing_devices:
            if self.capturing_devices[dev].device_class == device_id:
                self.capturing_devices[dev].set_device_filter(device_filter)
    ...
    def set_compatibility_device(self, device_path, compatibility_state):
        ...
        if compatibility_state and device_path not in self.devices_config[
            DEVICES_CONFIG_COMPATIBILITY_DEVICE_KEY
        ]:
            self.devices_config[DEVICES_CONFIG_COMPATIBILITY_DEVICE_KEY].append(
                device_path)
        elif not compatibility_state and device_path in self.devices_config[
            DEVICES_CONFIG_COMPATIBILITY_DEVICE_KEY
        ]:
        ...
        for device in self.devices:
            if device["id"] in self.devices_config:
                if CAPTURE_ELEMENT in self.devices_config[device["id"]]:
                    device[CAPTURE_ELEMENT] = self.devices_config[
                        device["id"]][CAPTURE_ELEMENT]
                else:
                    device[CAPTURE_ELEMENT] = False
                if FILTER_ELEMENT in self.devices_config[device["id"]]:
                    device[FILTER_ELEMENT] = self.devices_config[
